...
* 7 Layers each describes a specific networking component
	...
	- Layer 6: Presentation (==Encoding== - ==Serialization==)
		...
		- TLS Encryption / Decryption also happens here
	...
	- Layer 4: Transport (UDP / TCP)
		...
		- We add the source and destination ports here
...
- Layer 6: Presentation
	- Serializing JSON to ==flat byte string== 
		...
		- 1 character != 1 byte
		- For example: 🚀 has a Unicode of **U+1F680** (128640)
			- Convert to binary: 000011111011010000000
			- Use UTF-8 Template (Refer to table below)
...
| Byte 2 | 10xxxxxx         | 6 bits   | 10==011111==             | 9F                            |
| Byte 3 | 10xxxxxx         | 6 bits   | 10==011010==             | 9A                            |
| Byte 4 | 10xxxxxx         | 6 bits   | 10==000000==             | 80                            |
- Layer 5: Session
	- Request to establish TCP and TLS connections
	- Pause this request until 3 way handshake
- Layer 4: Transport
	- Include source and destination ports
	- Send ==SYN request== to port 443 (default port for https)
		- A SYN (Synchronize) request is the initial TCP packet sent from a client to a server to establish a network connection
...
- Let's animate the examples above (From L to L' where L' is the receiver)
	- **POST REQUEST**: L7 -> L6 -> L5 (No Connection, wait)
		- L5 (Request Session) -> L4 (send SYN) -> L3 -> L2 -> L1 ===> L'1 -> L'2 -> L'3 -> L'4 -> L'5 (Accept Connection)->L'4 (send SYN + ACK) -> L'3 -> L'2 -> L'1 ===> L1 -> L2 -> L3 -> L4 -> L5 (Accept) -> L4 (send ACK) -> L3 -> L2 -> L1 ===> L'1 -> L'2 -> L'3 -> L'4 -> L'5 (Connection Established)
	- Continue (L4 -> L3 -> L2 -> L1 ===> L'1 -> L'2 -> L'3 -> L'4 -> L'5 -> L'6 -> L'7)
 ...
 * When requesting a remote server IP, who decides the shortest path to destination ip and how does it know it? and at which layer does this happen?
	 * Who decides? The router
	 * In which layer? Layer 3
	 * How? Many ways can do that
		 * Routing table: A "map" or database stored in the router’s memory that lists known networks and the best interface to reach them
		 * Routing Engine: The software process that runs algorithms to calculate which path is the most efficient
	* See [this Gemini conversation](https://gemini.google.com/share/56702bea01fe) for more details
* What happens to source and destination IPs as well as source and destination MACs after each hop?
	* IPs never change
	* Source MAC:
		* It's set to the client MAC when firing the request
		* After each hop, it changes to the current  router's MAC
		* After the last hop, it changes to the target  router's MAC
	* Destination MAC
		* It's set to the current router's MAC
		* At the last hop, the **Destination MAC address is set to the MAC address of the actual destination device (the Server)**
		* See [this Gemini conversation](https://gemini.google.com/share/422401f11c44) for more details
* Modem is the device that connects my network to the internet, is modem an internet gateway?
	* Modem + router is the internet gateway, where modem connects you to the internet (convert the signals from and to your devices), router allows inbound and outbound traffic
- If we fire a request to destination IP `<DEST_IP>`, assigned to Server `S1`, and just before the request leaving Layer 1 of the client, <DEST_IP> is now assigned to another server `S2`, what will happen in that case if not 3 way handshake is done yet? and What will happen if 3 way handshake is already established?
	- If no handshake is established, it will arrive to `S2`, but there will be a check that the port is open and listening, if no port is open it will send a TCP RST (Reset) packet, or the famous `Connection Refused` error
	- If 3 way handshake is done, then `S2` will send a RST since it doesn't know the session, which throws the errors `Connection Reset by Peer` or `Broken Pipe`
	- See [this Gemini conversation](https://gemini.google.com/share/017ec232f868) for more details
- In which layer happens the NATing?
	- Layer 3, unless NATing involves changing ports then in will happen in Layer 4